...
interface BreakEvenMessageProps {
  result: ComparisonResultType;
  currentDistance: number;
}

// Main comparison result showing which service is cheaper
export function ComparisonResult({ result }: ComparisonResultProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-xl p-6 text-center card-shadow animate-fade-in",
        result.cheaper !== "same" 
...
// Break-even message component
export function BreakEvenMessage({ result, currentDistance }: BreakEvenMessageProps) {
  const formatPrice = (value: number) => {
    return value.toLocaleString("ja-JP");
  };
